...
import 'package:flutter_finey/screens/social_widgets/ui/post_detail_screen.dart';
...
class _InstaFriendProfileScreenState extends State<InstaFriendProfileScreen> {
  ...
  void initState() {
    ...
    _repository.getCurrentUser().then((user) {
      _repository.fetchUserDetailsById(user.email).then((currentUser) {
        setState(() {
          currentuser = currentUser;
        });
      });
      _repository.checkIsFollowing(widget.name, user.email).then((value) {
        print("VALUE : ${value}");
        setState(() {
          isFollowing = value;
        });
    ...
    return FutureBuilder(
      ...
      builder: ((context, AsyncSnapshot<List<DocumentSnapshot>> snapshot) {
        if (snapshot.hasData) {
          return GestureDetector(
            child: Text(
              'Total visualizações ${snapshot.data.length} comentadas',
